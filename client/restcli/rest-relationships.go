...
func (c *RestClient) FetchMutualFriendsAndServers(target string) (*types.MutualResponse, *types.APIError, error) {
	...
	apiErr, err := rest.NewReq(&c.Config).Get("users/" + target + "/mutual").DoAndMarshal(&mr)
...
func (c *RestClient) AcceptFriendRequest(target string) (*types.User, *types.APIError, error) {
	var u *types.User
	apiErr, err := rest.NewReq(&c.Config).Put("users/" + target + "/friend").DoAndMarshal(&u)
	return u, apiErr, err
}
...
func (c *RestClient) DenyFriendRequestOrRemoveFriend(target string) (*types.User, *types.APIError, error) {
	var u *types.User
	apiErr, err := rest.NewReq(&c.Config).Delete("users/" + target + "/friend").DoAndMarshal(&u)
	return u, apiErr, err
}
...
func (c *RestClient) BlockUser(target string) (*types.User, *types.APIError, error) {
	var u *types.User
	apiErr, err := rest.NewReq(&c.Config).Put("users/" + target + "/block").DoAndMarshal(&u)
	return u, apiErr, err
}
...
func (c *RestClient) UnblockUser(target string) (*types.User, *types.APIError, error) {
	var u *types.User
	apiErr, err := rest.NewReq(&c.Config).Delete("users/" + target + "/block").DoAndMarshal(&u)
	return u, apiErr, err
}
...
func (c *RestClient) SendFriendRequest(d *types.DataSendFriendRequest) (*types.User, *types.APIError, error) {
	var u *types.User
	apiErr, err := rest.NewReq(&c.Config).Post("users/friend").Json(d).DoAndMarshal(&u)
	return u, apiErr, err
}
